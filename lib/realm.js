...
function addContestListener(callback) {
  let listener = (contests, changes) => {
    callback(contests.length > 0 ? contests[0] : null);
  };
  return open().then(realm => {
      const contests = realm.objects('Contest');
      contests.addListener(listener);
      /// return a cleanup function to be invoked to remove the listener
      return () => {
        contests.removeListener(listener);
      };
    });
}
